...
class Solution {
    public int compareVersion(String version1, String version2) {
        ...
        while(i < vs1.length || j < vs2.length){
            ...
            int v2 = 0;
            if(i < vs1.length){
                v1 = Integer.parseInt(vs1[i]);
                i++;
            }
            if(j < vs2.length){
                v2 = Integer.parseInt(vs2[j]);
                j++;
            }
            if(v1 < v2){
                return -1;
    ...
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for(int i = 0; i < houses.length; i++){
            ...
            if(left >= 0 && left < heaters.length){
                curr = Math.min(Math.abs(target - heaters[left]), curr);
            }
            if(left - 1 >= 0){
                curr = Math.min(Math.abs(target - heaters[left - 1]), curr);
            }
...
答案是线性的，可二分的，所以可以用二分来猜答案。需要写check函数来确定答案的范围。
...
class Solution {
 public static int maxDistance(int[] position, int m) {
        ...
        private static boolean check(int[] position, int mid, int m){
            int count = 1;
            int slow = 0;
            int fast = 0;
            while(fast < position.length){
                if(position[fast] - position[slow] < mid){
                    fast++;
                }else{
                    count++;
                    slow = fast;
...
class Solution {
    public int kthSmallest(int[][] matrix, int k) {
        ...
        //答案x 在[left, right]之间。使用二分查找
        //check函数就是当前小于当前mid的数量是否>=k， 如果check = true，证明 x <= mid 。为false，证明 x > mid
        while(left < right){
            int mid = ((right - left) >> 1) + left;
...
class Solution {
    ...
    private void solve(TreeNode root, int targetSum, int curr){
        if(root == null){
            return;
        }
        path.addLast(root.val);
        int next = curr + root.val;
        if(root.left == null && root.right == null && next == targetSum){
            result.add(new ArrayList<>(path));
        }
        solve(root.left, targetSum, next);
        solve(root.right, targetSum, next);
        path.removeLast();
    }
}
```
...
class Solution {
    ...
    private void solve(TreeNode root, int targetSum, long prev){
        if(root == null){
            return;
        }
        long curr = prev + root.val;
        if(curr == targetSum){
            result++;
        }
...
### [面试题12. 矩阵中的路径](https://leetcode.cn/problems/ju-zhen-zhong-de-lu-jing-lcof/)
...
class Solution {
        ...
        for(int i = 0; i < board.length; i++){
            for(int j = 0; j < board[0].length; j++){
                if(board[i][j] == word.charAt(0)){
                    visisted[i][j] = true;
                    if(solve(board, visisted, word, i, j, 0)){
                        return true;
                    }
                    visisted[i][j] = false;
                }
    ...
    private boolean solve(char[][] board, boolean[][] visisted, String word, int i, int j, int index){
        ...
        for(int[] dirc : directions){
            ...
            if(newX >= 0 && newX < board.length && newY >= 0 && newY < board[0].length && visisted[newX][newY] == false && word.charAt(index + 1) == board[newX][newY]){
                visisted[newX][newY] = true;
                if(solve(board, visisted, word, newX, newY, index + 1)){
                    return true;
                }
                visisted[newX][newY] = false;
            }
        }
        return false;
    }
            ...
            for(int i = 0; i < size; i++){
                ...
                for(int[] dir : directions){
                    ...
                    if(newX >= 0 && newX < grid.length && newY >=0 && newY < grid[0].length && grid[newX][newY] == 0){
                        ...
                        queue.offer(new int[]{newX, newY});
                        result = Math.max(grid[x][y], result);
                    }
                }
            }
        }
...
class Solution {
    public int nthUglyNumber(int n) {
        ...
        int p3 = 0;
        for(int i = 1; i < n; i++){
            int curr = Math.min(dp[p1] * 2, Math.min(dp[p2] * 3, dp[p3] * 5));
            dp[i] = curr;
            if(curr == dp[p1] * 2){
                p1++;
            }
...
class Solution {
    public int findSubstringInWraproundString(String p) {
        ...
        for(int i = 1; i < chs.length; i++){
            int prev = chs[i - 1];
            int curr = chs[i];
            if((curr + 26 - prev) % 26 == 1){
                dp[i] = dp[i - 1] + 1;
            }else{
                dp[i] = 1;
            }
            map[chs[i] - 'a'] = Math.max(map[chs[i] - 'a'], dp[i]);
        }
        int result = 0;
        for(int i = 0; i < 26; i++){
            result += map[i];
...
class Solution {
    public double[] dicesProbability(int n) {
        //dp代表：投第i个骰子，出现点数j的次数
        int dp[][] = new int[n + 1][n * 6 + 1];
        //初始状态：投第1个骰子，出现点数1,2,3,4,5,6的次数肯定为1
        for(int j = 1; j <= 6; j++){
            dp[1][j] = 1;
        }
...
class Solution {
    public int minFallingPathSum(int[][] matrix) {
        ...
        int result = Integer.MAX_VALUE;
        for(int j = 0; j < matrix[0].length; j++){
            result = Math.min(dp[matrix.length - 1][j], result);
        }
        return result;
    }
...
class Solution {
    public int minFallingPathSum(int[][] matrix) {
        ...
        for(int j = 0; j < matrix[0].length; j++){
            dp[0][j] = matrix[0][j];
            if(dp[0][j] < (firstMin == -1 ? Integer.MAX_VALUE : dp[0][firstMin])){
                secondMin = firstMin;
                firstMin = j;
            }else if(dp[0][j] < (secondMin == -1 ? Integer.MAX_VALUE : dp[0][secondMin])){
                secondMin = j;
            }
        }
        ...
        for(int i = 1; i < matrix.length; i++){
            ...
            for(int j = 0; j < matrix[0].length; j++){
                dp[i][j] = Integer.MAX_VALUE;               
                if(j != firstMin){
                    dp[i][j] = Math.min(dp[i][j], dp[i - 1][firstMin] + matrix[i][j]);
                }else{
                    dp[i][j] = Math.min(dp[i][j], dp[i - 1][secondMin] + matrix[i][j]);
                }

                if(dp[i][j] < (tempFirst == -1 ? Integer.MAX_VALUE : dp[i][tempFirst])){
                    tempSecond = tempFirst;
                    tempFirst = j;
                }else if(dp[i][j] < (tempSecond == -1 ? Integer.MAX_VALUE : dp[i][tempSecond])){
                    tempSecond = j;
                }
            }
            firstMin = tempFirst;
...
class Solution {
    public int[] findBall(int[][] grid) {
        //第j个球，在第i行时，位于第dp[i][j]列。  i == 0 时，球在空中
        ...
        for(int i = 1; i < dp.length; i++){
            for(int j = 0; j < dp[0].length; j++){
                ...
                if(dp[i - 1][j] == -1){
                    dp[i][j] = -1;
                }else if(grid[i - 1][dp[i - 1][j]] == 1){
                    //当前球会往右边跑
                    //如果往右边跑越界，说明会卡住
                    if(dp[i - 1][j] + 1 == dp[0].length){
                        dp[i][j] = -1;
                    }else if(dp[i - 1][j] + 1 <= dp[0].length && grid[i - 1][dp[i - 1][j] + 1] == -1){
                        //如果往右边不越界，但是右边的那列挡板为-1，也会被卡住
                        dp[i][j] = -1;
                    }else{
                        //否则往右边跑
                        dp[i][j] = dp[i - 1][j] + 1;
                    ...
                    if(dp[i - 1][j] == 0){
                        dp[i][j] = -1;
                    }else if(dp[i - 1][j] != 0 && grid[i - 1][dp[i - 1][j] - 1] == 1){
                        dp[i][j] = -1;
                    }else{
                        dp[i][j] = dp[i - 1][j] - 1;
                    }
...
class Solution {
    ...
    //使用当前汽油量，从当前位置，到目标位置，共有几种方式
    private int solve(int[] locations, int currIndex, int targetIndex, int currFuel){
        //缓存中已经有结果了
        if(cache[currIndex][currFuel] != -1){
            return cache[currIndex][currFuel];
        }
        //在当前位置没汽油了,看汽车在不在目的地
        if(currFuel == 0){
            //不在目的地
            if(currIndex != targetIndex){
                cache[currIndex][currFuel] = 0;
                return 0;
            }
            //在目的地
            if(currIndex == targetIndex){
                cache[currIndex][currFuel] = 1;
                return 1;
            }            
        }
     

        //在当前位置还有汽油，看看能不能去别的地方。
        boolean hasNext = false;
        if(currFuel != 0){
            for(int i = 0; i < locations.length; i++){
                if(i != currIndex){
                    int need = Math.abs(locations[i] - locations[currIndex]);
                    if(currFuel >= need){
                        hasNext = true;
                        break;
                    }
        ...
        //能去别的地方
        //注意这里：如果currIndex == targetIndex，也算一条路径，就是选择不去别的地方。
        int count = currIndex == targetIndex ? 1 : 0;
        if(hasNext){
            for(int i = 0; i < locations.length; i++){
                if(i != currIndex){
                    int need = Math.abs(locations[i] - locations[currIndex]);
                    if(currFuel >= need){
                        count += solve(locations, i, targetIndex, currFuel - need);
                        count = count % MOD;
                    }
                }
            }
        }
        cache[currIndex][currFuel] = count;
        return count;
    }
}
```
...
class Solution {
    ...
    //现在正处于[x, y]，并且最多可以移动moveCount次
    private int solve(int x, int y, int moveCount, int m, int n){
        if(cache[x][y][moveCount] != -1){
            return cache[x][y][moveCount];
        }
        if(moveCount == 0){
            cache[x][y][moveCount] = 0;
            return 0;
        }
        ...
        for(int[] dirc : directions){
            ...
            if(newX < 0 || newY < 0 || newX >= m || newY >= n){
                ...
                count += solve(newX, newY, moveCount - 1, m, n);
            }
            count = count % MOD;
        }
        cache[x][y][moveCount] = count;
        return count;
    }
}
```

### [688. 骑士在棋盘上的概率](https://leetcode.cn/problems/knight-probability-in-chessboard/)[688. 骑士在棋盘上的概率](https://leetcode.cn/problems/knight-probability-in-chessboard/)
...
class Solution {
    ...
    public double knightProbability(int n, int k, int row, int column) {
        ...
        return sovle(row, column, k, n);
    }
    //在n * n 的棋盘上， 骑士在[x, y] 还能走k次，还在棋盘上的概率。
    private double sovle(int x, int y, int k, int n){
        if(cache[x][y][k] != -1){
            return cache[x][y][k];
        }
        if(k == 0){
        ...
        for(int[] dir : dirctions){
            ...
            currReuslt += sovle(newX, newY, k - 1, n) / 8;
        }
        cache[x][y][k] = currReuslt;
        return currReuslt;
    }
}
```
...
class Solution {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        ...
        for(List<Integer> currSpecial : special){
            int currPrice = 0;
            for(int i = 0; i < n; i++){
                currPrice += currSpecial.get(i) * price.get(i);
            }
            if(currPrice > currSpecial.get(n)){
                filterSpecial.add(currSpecial);
            }
        }

        return solve(price, filterSpecial, needs);

    }

    //需要购买currNeeds 物品所需最小的钱
    private int solve(List<Integer> price, List<List<Integer>> special, List<Integer> currNeeds){
        if(!map.containsKey(currNeeds)){
            int minPrice = 0;
            //1.不买大礼包
            for(int i = 0; i < n; i++){
                minPrice += price.get(i) * currNeeds.get(i);
            }
            ...
            for(List<Integer> currSpecial : special){
                boolean isBuy = true;
                for(int i = 0; i < n; i++){
                    //买当前大礼包，超出了清单指定数量，就不买当前的。
                    if(currNeeds.get(i) < currSpecial.get(i)){
                        isBuy = false;
                        break;
                    }
                }
                ...
                if(isBuy){
                    ...
                    for(int i = 0; i < n; i++){
                        nextNeeds.add(currNeeds.get(i) - currSpecial.get(i));
                    }
                    //买当前大礼包，递归，更新minPrice
                    minPrice = Math.min(minPrice, solve(price, special, nextNeeds) + currSpecial.get(n));
                }
            }
            return minPrice;
        }
...
class Solution {
    public boolean canMeasureWater(int x, int y, int target) {
        ...
        while(!stack.isEmpty()){
            ...
            set.add(hash(currState));
            int remainX = currState[0];
            int remainY = currState[1];
            ...
            stack.push(new int[]{remainX + Math.min(x - remainX, remainY), remainY - Math.min(x - remainX, remainY)});
        }
        return false;
    }

    private long hash(int[] stack){
        return (long) stack[0] * 1000001 + stack[1];
    }
...
DD大神：https://www.cnblogs.com/jbelial/articles/2116074.html
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(solve0(N, V, v, w));;
    }
    //N件物品，容量为V的背包，每件物品只能使用一次。  第 i 件物品的体积是 v[i]，价值是 w[i]。
    //第i件物品可有两种情况，选和不选。
    public static int solve0(int N, int V, int[] v, int[] w) {
    ...
    public static int solve1(int N, int V, int[] v, int[] w) {
        int[] dp = new int[V + 1];
        for(int i = 0; i < N; i++){
            //01背包需要从后向前遍历，因为放当前物品需要从前面转移过来，不放当前不物品就是其本身，因为最多只能放一次，要防止被覆盖
            for(int j = V; j >= v[i]; j--){
...
public class Main {
    ...
    //N件物品，容量为V的背包，每件物品只能使用一次。  第 i 件物品的体积是 v[i]，价值是 w[i]。
    //第i件物品可有多种情况，不选，选一个，选两个。。。。。
    //dp[i][j] = Math.max(dp[i - i][j], dp[i - 1][j - v[i]] + w[i], dp[i - 1][j - 2*v[i]] +  2*w[i], ......)
    //因为：dp[i][j - v[i]] =  Math.max(dp[i - 1][j - v[i]], dp[i - 1][j - 2*v[i]] + *w[i], dp[i - 1][j - 3*v[i]] + 2*w[i], ......)
    //所以：dp[i][j] = Math.max(dp[i - 1][j], dp[i][j - v[i]] + w[i]);
    public static int solve0(int N, int V, int[] v, int[] w) {
        int[][] dp = new int[N + 1][V + 1];
        for(int i = 1; i <= N; i++){
            for(int j = 0; j <= V; j++){
...
#### [**HJ16购物单**](https://www.nowcoder.com/practice/f9c6f980eeec43ef85be20755ddbeaf4?tpId=37&tqId=21239&rp=1&ru=/exam/oj/ta&qru=/exam/oj/ta&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37&difficulty=undefined&judgeStatus=undefined&tags=&title=)
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if(father == -1){
                w[i][0] = currCost;
                v[i][0] = currCost * currValue * 10;
            }else{
                for(int j = 1; j <= 2; j++){
                    if(w[father][j] == 0 && j == 1){
                        w[father][0] = arr[father][0];
                        v[father][0] = arr[father][0] * arr[father][1] * 10;
                        w[father][1] = w[father][0] + currCost;
                        v[father][1] = v[father][0] + currCost * currValue * 10;
                        break;
                    }else if(w[father][j] == 0 && j == 2) {
                        w[father][2] = w[father][0] + currCost;
                        v[father][2] = v[father][0] + currCost * currValue * 10;
                        w[father][3] = w[father][1] + w[father][2] - w[father][0];
                        v[father][3] = v[father][1] + v[father][2] - v[father][0];
                        break;
                    }
                }
            }
...
class Solution {
    public int numRollsToTarget(int n, int k, int target) {
        //投掷i个骰子，总和为j的方案总数
        //假设当前为第i个骰子，投出了1：dp[i][j] = dp[i - 1][j - 1]
        //投出了2： dp[i][j] = dp[i - 1][j - 2];
        //......
        //dp[i][j] = 上面的总和
        ...
        for(int a = 1; a <= n; a++){
            ...
            for(int b = 0; b <= target; b++){
                ...
                for(int c = 1; c <= k; c++){
                    if(b - c >= 0){
                        dp[a][b] = (dp[a][b] + dp[a - 1][b - c]) % 1000000007;
                    }
                }
            }
        }
...
class Solution {
    ...
    private void solve(int n, int currLeft, int currRight){
        if(currRight == n){
        ...
        if(currLeft < n){
            sb.append('(');
            solve(n, currLeft + 1, currRight);
            sb.deleteCharAt(sb.length() - 1);
        }

        if(currRight < currLeft){
            sb.append(')');
            solve(n, currLeft, currRight + 1);
            sb.deleteCharAt(sb.length() - 1);
        }
    }
}
...
class Solution {
	...
	//计算分数
    private void solve(String curr, int score, int index){
        ...
        if(index == origin.length()){
            if(score == 0){
                //判断当前字符串是否最大
                if(curr.length() > currMaxLen){
                    set = new HashSet<>();
                    currMaxLen = curr.length();
                    set.add(curr);
                }else if(curr.length() == currMaxLen){
                    set.add(curr);
                }
            }
            return;
        ...
        if(origin.charAt(index) == '('){
            solve(curr + origin.charAt(index), score + 1, index + 1);
            solve(curr, score, index + 1);
        }else if(origin.charAt(index) == ')'){
            solve(curr + origin.charAt(index), score - 1, index + 1);
            solve(curr, score, index + 1);           
        }else{
            solve(curr + origin.charAt(index), score, index + 1);
        }
...
class Solution {
    ...
    private boolean solve(int n, int currIndex){
        if(currIndex == (1 << n)){
            boolean flag = false;
            int first = list.get(0);
            int second = list.get((1 << n) - 1);
        ...
        int prev = list.get(currIndex - 1);
        for(int i = 0; i < n; i++){
            int next = prev ^ (1 << i);
            if(!set.contains(next)){
                set.add(next);
                list.add(next);
                if(solve(n, currIndex + 1)){
                    return true;
                }
                set.remove(next);
...
class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        return ((bucktes - 1) * (n + 1) + count) >= tasks.length ? ((bucktes - 1) * (n + 1) + count) : tasks.length;
    }
}
```
...
class Solution {
    public String reorganizeString(String s) {
        ...
        for(int i = 0; i < chs.length; i++){
            map[chs[i] - 'a'][1]++;
            maxCount = Math.max(map[chs[i] - 'a'][1], maxCount);
        }

        if(maxCount > chs.length - maxCount + 1){
            return "";
        }
...
### [358. K 距离间隔重排字符串](https://leetcode-cn.com/problems/rearrange-string-k-distance-apart/)（未做）
...
class Solution {
    public boolean canReorderDoubled(int[] arr) {
        ...
        for(int x : list){
            ...
            if(countX != 0){
                if(countX > map.getOrDefault(2 * x, 0)){
                    return false;
                }
                map.put(2 * x, map.get(2 * x) - countX);
            }
        }
        return true;
    }
...
class Solution {
    public int findMinDifference(List<String> timePoints) {
        ...
        int t0 = getMinute(timePoints.get(0));
        int prev = t0;
        int result = Integer.MAX_VALUE;
        ...
        result = Math.min(result, t0 + 1440 - prev);
        return result;
...
class Solution {
    public int minimumDeviation(int[] nums) {
        ...
        while((set.last() % 2 == 0) &&(result > 0)){
            int currMax = set.last();
            int currMin = set.first();
            set.remove(currMax);
...
class Solution {
    public int[] singleNumbers(int[] nums) {
        ...
        int temp = 0; 
        //1.首先全部异或,得到temp = a ^ b，随便在temp找到一位为1，说明 a 和 b在该位上不同
        for(int i = 0; i < nums.length; i++){
            temp = (temp ^ nums[i]);
        }
        //得到该位，那么根据该位可以把数组分成两组，一定有x和y被分到两组
        int m = 1;
        while((m & temp) == 0){
            m = m << 1;
        }
...
class Solution {
    public int divide(int dividend, int divisor) {
        ...
        int count = solve(dividend, divisor);
        if(count == Integer.MIN_VALUE && isNegative == false){
    ...
    private int solve(int a, int b){
        ...
        int curr = b;
        //判断一下书否可以快速除，以及是否溢出
        while(a < (curr << 1) && (curr << 1) < 0){
            curr = curr << 1;
            count = count << 1;
        }
        return count + solve(a - curr, b);
    }
}
```
...
## [ 质数因子](https://www.nowcoder.com/practice/196534628ca6490ebce2e336b47b3607?tpId=37&tqId=21229&rp=1&ru=/exam/oj/ta&qru=/exam/oj/ta&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37&difficulty=undefined&judgeStatus=undefined&tags=&title=)
...
```java
//遍历两边，第一次删除不正确的），第二次删除多余的（，第二次需要从后往前遍历
class Solution {
...
现在有n个工厂和一个数组cost，cost[i]代表第i个工厂生产一个零件要cost[i]个小时，请问生产y个零件最少需要几个小时，工厂可以同时开工。
...
假设现在有n个筐从左到右排列，每个筐最多可以放一个苹果，也可以不放。如果在一个筐中放了一个苹果，那么这个筐相邻的筐不能放苹果。问有几个方法。
...
public static int solve(int n){
	...
	//当前可以放或者不放
    for(int i = 1; i < n; i++){
        int curr0 = dp[0] + dp[1];  //当前不放，前面可以放也可以不放
        int curr1 = dp[0];	//当前放，前面只能不放
        dp[0] = curr0;
        dp[1] = curr1;
    }
    return dp[0] + dp[1];
...
给定一个数n，如：23121；给定一组数字如[2, 4, 9]，求a中元素组成的小于n的最大数
    ...
    public static void main(String[] args) {
        ...
        while (i < chs.length) {
            ...
            for (int j = 0; j < arr.length; j++) {
                if (i != chs.length - 1 && curr >= result[i] && curr >= arr[j]) {
                    result[i] = arr[j];
                } else if (i == chs.length - 1 && curr > result[i] && curr > arr[j]) {
                    result[i] = arr[j];
                }
            }
            if (i != 0) {
                if (result[i - 1] < chs[i - 1] - '0') {
                    int ans = 0;
                    for (int j = 0; j < i; j++) {
                        ans = ans * 10 + result[j];
                    }
...
### [327. 区间和的个数(待学)](https://leetcode.cn/problems/count-of-range-sum/)
...
class Solution {
    public String addBoldTag(String s, String[] words) {
        ...
        for(int i = 0; i < mark.length; i++){
            if(mark[i] == true){
                if(i == 0 || mark[i - 1] == false){
                    sb.append("<b>");
                }
            }
            sb.append(s.charAt(i));
            if(mark[i] == true){
                if(i == mark.length - 1 || mark[i + 1] == false){
                    sb.append("</b>");
                }
            }
        }
